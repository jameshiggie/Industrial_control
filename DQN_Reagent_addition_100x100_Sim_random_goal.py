...
class env:
    def __init__(self):
        ...
        self.colour = [[255,0,0], [0,255,0],[0,0,255],[255,255,255], [255,255,255] ]  #source tank 1 and 2, then the destination tank colour
        self.level = [100,100,100,100,0]                              #source tank 1 and 2, then the destination tank levels
        self.tank_info = {'st1': [120,20,0,[255,255,255]],
                          'st2': [120,40,1, [255,255,255]],
                          'st3': [120,60,2, [255,255,255]],
                          'st4': [120,80,3, [255,255,255]],
                          'dt1': [120,100,4, [255,255,255]] 
                         } 
    ...
    def action(self, choice):
        ...
        self.level[4] = self.level[4]+1
        #deliver to destination tank
        for j in range(3):
            self.colour[4][j] = self.colour[4][j]*((self.level[4]-1)/self.level[4]) + self.colour[choice][j] * (1/self.level[4]) 
    ...
    def step(self, action):
        ...
        c=0
        for i in range(3):
            c += abs(GOAL['colour'][i] - self.colour[4][i])


        if self.level[4] > 100:
            reward = -self.Tank_overfill_penalty
            done = True
        if self.level[4] > GOAL['volume']:     #might have to put something in about colour too...
            reward = -self.Tank_overfill_penalty
            done = True
        
        if (self.colour[4])[0] >= GOAL['colour'][0]*0.9 and (self.colour[4])[0] <= GOAL['colour'][0]*1.1 and \
            (self.colour[4])[1] >= GOAL['colour'][1]*0.9 and (self.colour[4])[1] <= GOAL['colour'][1]*1.1 and \
                (self.colour[4])[2] >= GOAL['colour'][2]*0.9 and (self.colour[4])[2] <= GOAL['colour'][2]*1.1 :
                if self.level[4] == GOAL['volume']:
                    reward = self.Complete_mixture
                    done = True
                    good = True
        else:
            reward = - c/20 - abs(self.level[4]-GOAL['volume']) 
...
class DQNAgent:
    ...
    def create_model(self):
        ...
        model.add(Dense(env.Action_space, activation='linear'))  # action space = 3
...
for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
    ...
    #requiered prodcut
    GOAL = {'colour': [random.randint(0,255),random.randint(0,255),random.randint(0,255)],
            'volume': random.randint(0,60)
            }
