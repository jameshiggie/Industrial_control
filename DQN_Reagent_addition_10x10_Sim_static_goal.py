...
class env:
    def __init__(self):
        ...
        self.level = [10,10,0]                              #source tank 1 and 2, then the destination tank levels
        self.tank_info = {'st1': [20,10,0,[255,255,255]],
                          'st2': [20,30,1, [255,255,255]],
                          'dt1': [38,20,2, [255,255,255]] 
                         } 
    ...
    def step(self, action):
        ...
        if self.level[2] >= GOAL['volume']:     #might have to put something in about colour too...
            reward -= self.Tank_overfill_penalty/10
        
        if (self.colour[2])[0] >= GOAL['colour'][0]*0.9 and (self.colour[2])[0] <= GOAL['colour'][0]*1.1 and \
            (self.colour[2])[1] >= GOAL['colour'][1]*0.9 and (self.colour[2])[1] <= GOAL['colour'][1]*1.1 and \
                (self.colour[2])[2] >= GOAL['colour'][2]*0.9 and (self.colour[2])[2] <= GOAL['colour'][2]*1.1 :
                if self.level[2] <= GOAL['volume']*1.05 and self.level[2] >= GOAL['volume']:
                    reward = self.Complete_mixture

        else:
            reward -= self.Consumable_usage_cost

        done = False
        if reward >= self.Complete_mixture or reward <= -self.Tank_overfill_penalty or self.episode_step >= 200:
            print(self.colour[2], self.level[2])
            done = True
    ...
    def get_image(self):
        ...
        for tank in self.tank_info:
            ...
            #Side of tank
            for i in range(11):
                env_plot[y-11+i][x-1:x] = colour_convert(tank_wall_colour)
            for i in range(11):
                env_plot[y-11+i][x+10:x+10+1] = colour_convert(tank_wall_colour)
            for i in range(1):
                env_plot[y+i][x-1:x+10+1] = colour_convert(tank_wall_colour)

            
            if tank == 'dt1':
                #Goal example
                for l in range(GOAL['volume'],0,-1):
                    env_plot[y-l][x+15:x+25] = colour_convert(GOAL['colour'])
                #Side of tank
                for i in range(11):
                    env_plot[y-11+i][x-1+15:x+15] = colour_convert(tank_wall_colour)
                for i in range(11):
                    env_plot[y-11+i][x+25:x+1+25] = colour_convert(tank_wall_colour)
                for i in range(1):
                    env_plot[y+i][x-1+15:x+25+1] = colour_convert(tank_wall_colour)
